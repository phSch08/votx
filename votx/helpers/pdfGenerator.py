...
def generateRegistrationPDF(headline1: str,
                            headline2: str,
                            dateline: str,
                            tokenDescription: str,
                            url: str,
                            tokenList: list[tuple[str, list[str]]]):
    ...
    for idx, token in enumerate(tokenList):
        ...
        pdf.cell(w=210, h=15, text=url,
                 align="C", new_x="LEFT", new_y="NEXT")
        pdf.set_font("helvetica", style="", size=12)
        pdf.cell(w=210, h=5, new_x="LEFT", new_y="NEXT")

        pdf.cell(w=100, h=7, text=tokenDescription, align="C",
...
def generateBallotProtocol(
    ...
    class PDF(FPDF):
        ...
        def header(self):
            ...
            self.set_xy(15, 0)
            self.cell(w=120, h=25, text="Wahlprotokoll",
                    align="L", new_x="LEFT", new_y="NEXT")
            
            self.image("votx/static/logo.svg", x=180, y=5, w=20, h=15, keep_aspect_ratio=True)
            self.set_xy(20,30)
    ...
    pdf.multi_cell(w=100, text=ballotTitle, new_y="NEXT")
    ...
    for r in results:
        ...
        pdf.cell(h=6, w=30, text=f"Stimmen: {
                 r[1]}", new_x="RIGHT", new_y="TOP")
        pdf.set_font("helvetica", style="", size=11)
    ...
    for e in events:
        ...
        pdf.set_font("helvetica", style="I", size=7)
        pdf.set_text_color(100)
        pdf.cell(h=6, w=30, text=e[0].strftime(
            "%d.%m.%y %H:%M:%S"), new_x="RIGHT", new_y="TOP")
        pdf.set_font("helvetica", style="", size=11)
        pdf.set_text_color(0)
